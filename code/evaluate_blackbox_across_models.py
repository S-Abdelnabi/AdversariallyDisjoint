...
def fgsm_attack(model_src, target, image, epsilon):
    output_src = model_src(image)
...
def fgm_attack(model_src, target, image, epsilon):
    output_src = model_src(image)
    loss = F.cross_entropy(output_src, target)
    model_src.zero_grad()
    loss.backward()
...
def mi_fgsm(model, images, labels, eps, decay, iters=0, alpha=0) :
...
def r_fgsm_attack(model, labels, images, eps, alpha) :
...
def pgd_random(model, images, labels, eps, alpha, iters=0) :
    images = images.to(device)
    labels = labels.to(device)
    clamp_max = 1.0
...
#ead attack 
def ead_attack(model, images, labels, beta, decision_rule, targeted=False, c=1e-4, kappa=0, max_iter=1000, learning_rate=0.01) :
    def ISTA(new, old):
        with torch.no_grad():
            diff = new - old
...
def test_attack( models, src_index, device, test_loader, epsilon, split_type):
    # Accuracy counter
    ...
    for data, target in test_loader:
        ...
        if args.attack_type == 'fgsm':        
            perturbed_data = fgsm_attack(models[src_index], target,data, epsilon)
        elif args.attack_type == 'rfgsm':        
            perturbed_data = r_fgsm_attack(models[src_index], target,data, epsilon, epsilon/2)
        elif args.attack_type == 'fgm':
            perturbed_data = fgm_attack(models[src_index], target, data, epsilon)
                        
        elif args.attack_type == 'mifgsm':
            perturbed_data = mi_fgsm(models[src_index], data, target, args.epsilon, args.decay, args.mi_itrs,args.alpha)
            
        elif args.attack_type == 'pgd':
            perturbed_data = pgd_random(models[src_index], data, target, args.epsilon, args.alpha, args.bim_itrs)
            
        elif args.attack_type == 'cw':
            perturbed_data = cw_l2_attack(models[src_index], data, target, targeted=False, c=args.cw_c, kappa=args.cw_conf, max_iter=args.cw_iters, learning_rate=args.cw_lr)            
        elif args.attack_type == 'ead':        
            perturbed_data = ead_attack(models[src_index], data, target, args.ead_beta, args.ead_rule, targeted=False, c=args.ead_c, kappa=args.ead_conf, max_iter=args.ead_iters, learning_rate=args.ead_lr)
        else:
...
for i in range(0,args.models_num):
    print('********* Model '+str(i)+' *********')
    acc_test[i,:] = test_attack(models_list,i, device, test_loader, args.epsilon, 'Test')*100 
    print(acc_test[i,:])
print('*** Test ***')
print(acc_test)
print('test avg: '+str(np.mean( [acc_test[i][j] for i in range(args.models_num) for j in range(args.models_num) if i!=j])))
print('test std: '+str(np.std( [acc_test[i][j] for i in range(args.models_num) for j in range(args.models_num) if i!=j])))
print('test min: '+str(np.min( [acc_test[i][j] for i in range(args.models_num) for j in range(args.models_num) if i!=j])))
print('average: '+str(np.mean(acc_test)))
