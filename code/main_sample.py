...
import os
import numpy as np
from utils import blackbox_allmodels, select_valsubset, test_acc_allmodels, blackbox_external_model,whitebox_allmodels
...
parser.add_argument('--accumulate_steps', type=int, default=1, metavar='N',
                        help='how many iteration step before taking an optimizer step')
parser.add_argument('--divide_steps', type=int, default=1, metavar='N',
                        help='divide lr_steps by a factor')
...
parser.add_argument('--checkpt_dir', type=str, default='checkpt_dir',
                        help='Directory for loading checkpts')
parser.add_argument('--continue_tr', type=int, default='0',
                        help='continue training from a checkpt')
parser.add_argument('--checkpt', type=str, default='_best',
                        help='which checkpoint to load')
parser.add_argument('--check_whitebox', type=int, default=0,
                        help='whether to check whitebox')
...
def pairwise_gradient_transfer(data, targets, data_grads_list, old_losses, epsilon=1,random_sample=[]):
    loss_avg = 0
    new_losses = []
    count = 0
    if len(random_sample) > 1:   
        models_subset = [models_list[i] for i in random_sample]
        grads_subset = [data_grads_list[i] for i in random_sample]
        old_losses_subset = [old_losses[i] for i in random_sample]     
    else:
        models_subset = models_list
        grads_subset = data_grads_list
        old_losses_subset = old_losses
    for i in range(0,len(models_subset)):
        for j in range(0,len(models_subset)):
            if i==j:
                continue
            diff_loss_clampled = compute_loss_increase(models_subset[i],data,targets,old_losses_subset[i],grads_subset[j],epsilon)
            loss_avg = diff_loss_clampled.detach().data + loss_avg             
            new_losses.append(diff_loss_clampled)
            count = count + 1
    mean_loss = loss_avg / count            
    return new_losses,mean_loss
...
def load_models():
    ...
    for i in range(0,args.models_num):    
        ...
        model1.cuda()
        model1.load_state_dict(torch.load(args.checkpt_dir+'/model'+str(i)+args.checkpt+'.pt'))
        model1.eval()
...
def train(train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        full_range = np.arange(args.models_num)    
        np.random.shuffle(full_range)   
        random_sample = full_range[0:3] 
        ...
        if args.include_transfer_loss == 1 and epoch > args.start_transfer_loss:   
            transfer_loss, transfer_losses_avg = pairwise_gradient_transfer(data,target,data_grads_list,losses_list,args.fgm_epsilon_training,random_sample=random_sample)   
            sum_transfer_loss += transfer_losses_avg 
            
        if args.include_transfer_sgn_loss == 1 and epoch > args.start_transfer_sgn_loss:   
            sgn_transfer_loss, sgn_transfer_losses_avg = pairwise_gradient_transfer(data,target,data_sign_grads_list,losses_list,args.fgsm_epsilon_training,random_sample=random_sample)   
            sum_sgn_transfer_loss += sgn_transfer_losses_avg          
...
def test(test_loader, split_type):
    ...
    for data, target in test_loader:
        ...
        with torch.no_grad():
            ...
            if args.include_transfer_loss == 1 and epoch > args.start_transfer_loss:   
                transfer_loss,transfer_loss_avg = pairwise_gradient_transfer(data,target,data_grads_list,losses_list,args.fgm_epsilon_training)
                test_loss_transfer += transfer_loss_avg
            if args.include_transfer_sgn_loss == 1 and epoch > args.start_transfer_sgn_loss:   
                sgn_transfer_loss,sgn_transfer_loss_avg = pairwise_gradient_transfer(data,target,data_sign_grads_list,losses_list,args.fgsm_epsilon_training)
                test_loss_sgn_transfer += sgn_transfer_loss_avg   
...
if args.continue_tr == 0:
    models_list = create_models()
elif args.continue_tr == 1:
...
lr_steps = args.epochs*len(train_loader)/args.divide_steps
print(lr_steps)
...
for epoch in range(1, args.epochs + 1):
    ...
    if epoch % args.tranfser_check_interval == 0:
        ...
        if args.check_whitebox == 1:
            whitebox_acc = whitebox_allmodels(models_list, device, val_loader, 0.031, 'pgd',0.0078,7)
            print('Val subset: average whitebox PGD: {:.4f}\n'.format(whitebox_acc))
        ...
        log_file_val.write(str(nonadv_acc_subset) + ', '+ str(transferable_adv_acc_subset) + str(whitebox_acc)+'\n' if args.check_whitebox == 1 else str(nonadv_acc_subset) + ', '+ str(transferable_adv_acc_subset) + '\n') 
        log_file_val.flush()
    ...
    if args.check_whitebox == 1:
        both_acc =  transferable_adv_acc_subset + whitebox_acc
